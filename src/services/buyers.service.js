...
export const BuyersService = {
  ...
  async bulkUpload(filename,buffer,userId){
    if(!/\.xlsx$/i.test(filename))throw Object.assign(new Error("Only .xlsx Buyer files are supported."),{status:400});
    const input=await parseBuyerWorkbook(buffer),client=await getClient();const stats={rows:input.length,groupsCreated:0,groupsUpdated:0,locationsCreated:0,locationsUpdated:0,contactsCreated:0,contactsReused:0,linksCreated:0,skipped:0,errors:[]};let summaryBuyerId=null;
